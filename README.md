...
`tools/list` at `https://gateway.pipeworx.io/opencages/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/opencages_geocode \
  -H 'Content-Type: application/json' \
  -d '{"query":"Paris, France"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/opencages_geocode`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
